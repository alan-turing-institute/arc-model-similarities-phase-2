...
where `"drop-only"` is an example of an exerperiment group with a named config (see below for more information on configs). It will generate bash scripts in `ROOT/metrics_scripts/`. Note additional optional arguments that can be passed to the generation script include `--dataset_config_path` and `--metrics_config_path`.
...
where `"drop-only"` is an example of an exerperiment group with a named config (see below for more information on configs). It will generate bash scripts in `ROOT/train_scripts/`. Note additional optional arguments to the generation script include `--dataset_config_path`, `--metrics_config_path`, `--account_name`, and `--conda_env_path` (see below).
...
where once again `drop-only` is an example of an exerperiment group with a named config.
...
where `"drop-only"` is an example of an exerperiment group with a named config (see below for more information on configs). It will generate bash scripts in `ROOT/attack_scripts/`. Note additional optional arguments include `--dataset_config_path`, `--trainer_config_path`, and `--attack_config_path` (see below).
...
- each transforms entry should contain an array, where each entry in the array is a single tranform, with name and kwarg arguemnts
